<template>
  <div>
    ...
    <div>
      ...
      <select v-model="selectSubCity">
        <option v-for="subCity in subCities[selectCity]">{{ subCity }}</option>
      </select>
      <select v-model="place">
        <option v-for="bankName in places">{{ bankName.place_name }}</option>
      </select>
    </div>
    <div id="map" style="width: 500px; height: 400px"></div>
    <ul>
      <li v-if="place.length > 0">{{ placeInfo }}</li>
    </ul>
  </div>
</template>
...
const subCities = ref(
  {
    ...
    '경기도': [
      '가평군', '고양시', '과천시', '광명시', '광주시', '구리시', '군포시', '김포시', '남양주시', '동두천시', '부천시', '성남시', '수원시', '시흥시', '안산시',
    ...
    '경상북도': [
      '경산시', '경상북도', '경주시', '고령군', '구미시', '김천시', '문경시', '봉화군', '상주시',
      '성주군', '안동시', '영덕군', '영양군', '영주시', '영천시', '예천군', '울릉군', '울진군', '의성군', '청도군', '청송군', '칠곡군', '포항시'
    ],
    '광주광역시': [
      '광산구', '남구', '동구', '북구', '서구'
    ],
    '대구광역시': [
      '군위군', '남구', '달서구', '달성군', '대구광역시', '동구', '북구', '서구', '수성구', '중구'
    ],
    '대전광역시': [
      '대덕구', '동구', '서구', '유성구', '중구'
    ],
...
watch((place), (newValue) => {
  if (place.value.length > 0) {
    placeInfo.value = places.value.filter((element) => element.place_name === place.value)
  }
})
</script>
